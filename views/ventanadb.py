...
from database.db_Gestor import crear_db, eliminar_db, hacer_dump, hacer_restore_dump
...
class VentanaSecundaria(tk.Toplevel):
    def __init__(self):
        ...
        self.btn_hacer_dump = tk.Button(
            self.lbf_principal, text="Hacer Dump DB", command=self.generar_dump
        )
        self.btn_hacer_dump.grid(column=0, row=2, padx=10, pady=10)

        self.btn_hacer_restore_dump = tk.Button(
            self.lbf_principal,
            text="Hacer Restore \nDump DB",
            command=self.generar_restore_dump,
        )
        self.btn_hacer_restore_dump.grid(column=0, row=3, padx=10, pady=10)
    ...
    def generar_dump(self):
        hacer_dump()
        ms.showinfo("Ok", "Dump creado")

    def generar_restore_dump(self):
        hacer_restore_dump()
        ms.showinfo("Ok", "Dump restoreado creado")
